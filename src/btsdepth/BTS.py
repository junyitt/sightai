...
class BtsController:
    ...
    def predict(self, input, is_channels_first=True, focal=715.0873, normalize=False):
        ...
        if is_channels_first:
            tensor_input = torch.tensor(input).unsqueeze(-1).to(device).transpose(0, 3).transpose(2,
                                                                                                          3).transpose(
                1, 2)
        else:
            tensor_input = torch.tensor(input).unsqueeze(-1).to(device).transpose(0, 3).transpose(1,
                                                                                                          2).transpose(
                2, 3)
